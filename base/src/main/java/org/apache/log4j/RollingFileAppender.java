 ...
 */
  ...
  void rollOver() {
    ...
    if(maxBackupIndex > 0) {
    	...
    	file = FileOperate.getFile(fileName + '.' + maxBackupIndex);
    	if (file.exists())
    		renameSucceeded = file.delete();

    	// Map {(maxBackupIndex - 1), ..., 2, 1} to {maxBackupIndex, ..., 3, 2}
    	for (int i = maxBackupIndex - 1; i >= 1 && renameSucceeded; i--) {
    		file = FileOperate.getFile(fileName + "." + i);
    		if (file.exists()) {
    			target = FileOperate.getFile(fileName + '.' + (i + 1));
    	...
    	if(renameSucceeded) {
    		...
    		target = FileOperate.getFile(fileName + "." + 1);

    		this.closeFile(); // keep windows happy.

    		file = FileOperate.getFile(fileName);
    		LogLog.debug("Renaming file " + file + " to " + target);
    		renameSucceeded = file.renameTo(target);
    		//
  ...
  void setFile(String fileName, boolean append, boolean bufferedIO, int bufferSize)
    ...
    if(append) {
      File f = FileOperate.getFile(this.fileName);
